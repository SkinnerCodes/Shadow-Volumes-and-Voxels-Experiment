...
public class Node {
    ...
    public Voxel createVoxel(NodePosition targetPosition, int level, VoxelType voxelType, boolean shouldTryMerge)
    {
        ...
        if (parent instanceof Chunk)
        {
            ...
            if (chunk.level >= level + 1) {
                ...
                {
                    Octant octant = chunk.getChildPositionAsOctant(targetPosition.relPosition);
                    return chunk.createChildVoxel(octant, voxelType, shouldTryMerge);
                }
            } else {
                while (chunk.level < level + 1)
                    chunk = chunk.parent;
            ...
            if (v.type == voxelType)
                ...
                while (v.level > level) {
                    Chunk tempChunk = v.split();
                    Octant octant = tempChunk.getChildPositionAsOctant(targetPosition.relPosition);
                    v = (Voxel) tempChunk.children[octant.getValue()];
                }
                if (v.level == level) {
                    v.type = voxelType;
                    return v;
    ...
    //-------------------------------------------------------------------------------
    //  Travels all the way to the root chunk to invalidate the buffer within and force
    //  it to rebuild the buffer on the next draw step.
    //-------------------------------------------------------------------------------
